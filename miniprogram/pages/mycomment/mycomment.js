          ...
          this.setData({
           ...
           for (var i = 0; i < this.data.items.length; i++){
             for(var j = 0; j < this.data.items[i].dangkou.meal.length; j++){
               for(var k = 0; k < this.data.items[i].dangkou.meal[j].pj.length; k++){
                  if(this.data.items[i].dangkou.meal[j].pj[k].openid == this.data.openid){//把已收藏的菜品信息放入新数组
                   //用于数据绑定
                    var temp='meal1'+'['+h+']';
                   //用于传参
